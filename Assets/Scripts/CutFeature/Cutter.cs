...
public class Cutter : MonoBehaviour
{
    ...
    public static GameObject Cut(GameObject originalGameObject, Vector3 contactPoint, Vector3 cutNormal)
    {
        ...
        GeneratedMesh GMeshL = new GeneratedMesh();
        GeneratedMesh GMeshR = new GeneratedMesh();
        
        SeparateMeshes(GMeshL,GMeshR,cutPlane,addedVertices);
        FillCut(addedVertices, cutPlane, GMeshL, GMeshR);

        Mesh finishedLeftMesh = GMeshL.GetGeneratedMesh();
        Mesh finishedRightMesh = GMeshR.GetGeneratedMesh();
        ...
        foreach (var col in originalCols) Destroy(col);//clear colliders

        //we set LeftMesh as the original part.
        originalGameObject.GetComponent<MeshFilter>().mesh = finishedLeftMesh;
        var collider = originalGameObject.AddComponent<MeshCollider>();//readd
        collider.sharedMesh = finishedLeftMesh;
        collider.convex = true;

        //Simplify all material into one value 
        int materialAmountL = finishedLeftMesh.subMeshCount;
        Material firstOriginalMaterial = originalGameObject.GetComponent<MeshRenderer>().material;
        Material[] mats = new Material[materialAmountL];
        for (int i = 0; i < materialAmountL; i++) mats[i] = firstOriginalMaterial;
        ...
        //Simplify all material into one value
        int materialAmountR = finishedRightMesh.subMeshCount;
        mats = new Material[materialAmountR];
        for (int i = 0; i < materialAmountR; i++) mats[i] = firstOriginalMaterial;
        rightMeshObj.GetComponent<MeshRenderer>().materials = mats;
        
        rightMeshObj.AddComponent<MeshFilter>().mesh = finishedRightMesh;

        MeshCollider colliderR = rightMeshObj.AddComponent<MeshCollider>();
        colliderR.sharedMesh = finishedRightMesh;
        colliderR.convex = true;
        
        var rightRigidbody = rightMeshObj.AddComponent<Rigidbody>();
